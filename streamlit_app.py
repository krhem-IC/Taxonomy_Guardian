...
def normalize_column_headers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(columns_to_keep) != len(df.columns):
        ...
        log_event("INFO", "Removed empty unnamed columns", 
                  removed_count=len(df.columns) - len(columns_to_keep))
    
    # Normalize all column names to uppercase and strip whitespace
...
def read_user_file(file_name: str, file_bytes: bytes) -> pd.DataFrame:
    """Read user uploaded file with better error handling - FIXED"""
...
def brand_accuracy_cleanup(
    ...
    # Work on the dataframe directly to avoid unnecessary copying
